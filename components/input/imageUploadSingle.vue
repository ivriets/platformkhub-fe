        ...
        <div  v-if="dataImage && dataImage.displayImage === ''"  class="image-area cursor-pointer" @dragover="dragOver" @dragleave="dragLeave" @drop="drop">
            <div class="min-h-[150px] w-full border-dashed border-2 border-warna-tujuh rounded-lg pt-[9px] pb-[25px] flex items-center justify-center">
                <div class="text-center">
                    ...
                    <button @click="$refs.inputfiles.click()" title="Upload Image"  class="mx-auto bg-white border border-warna-tujuh rounded-md hover:bg-gray-200 hover:shadow-md transition-all shadow shadow-[#45a6ff33] py-2 w-[145px] text-center mx-auto cursor-pointer">Pilih File</button>
                </div>
            </div>
        </div>
...
export default {
    props: ['value', 'maxSize', 'accept', 'label', 'useCrop', 'cropRatio'],
    components: {
        Cropper
    },
    data() {
    ...
    methods: {
        initialize() {
          this.stencilProps.aspectRatio = this.cropRatio && this.cropRatio !== '' ? this.cropRatio : 1;
            ...
            if (this.value && (this.value.image !== '' && this.value.image !== undefined)) {
              // this.$emit('input', this.dataImage)
              // this.dataImage = this.value.image
              console.log('valImage',this.value.image)
              const forPath = this.value.image.substring(0,1) === '/' ? this.basePath : ''
              this.dataImage.displayImage = forPath + this.value.image
            } 
