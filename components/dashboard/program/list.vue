...
export default {
    ...
    methods: {
        ...
        async masterPoint() {
            ...
            await this.$apiPlatform.get('moderator/programs/?submission='+item.endpoint+'&limit='+this.limit+'&offset='+this.startIndex+'&title='+this.filter.search+'&sortbycreatedat='+this.sorter.createdAt).then(res => {
        ...
        async getLogProgram() {
            ...
            await this.$apiPlatform.get('moderator/').then(res => {
                const data = res.data.logProgram
                this.kapsul= [
                    {
                        id: 'all',
                        label: 'All',
